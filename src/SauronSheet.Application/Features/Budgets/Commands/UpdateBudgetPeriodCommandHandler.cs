...
/// <summary>
/// Handler for UpdateBudgetPeriodCommand.
/// Validates ownership, delegates to Budget.UpdateGranularity and Budget.UpdateLimit,
...
public class UpdateBudgetPeriodCommandHandler : IRequestHandler<UpdateBudgetPeriodCommand>
{
    ...
    public UpdateBudgetPeriodCommandHandler(IBudgetRepository budgetRepo, IUserContext userContext)
    {
        _budgetRepo = budgetRepo ?? throw new ArgumentNullException(nameof(budgetRepo));
        _userContext = userContext ?? throw new ArgumentNullException(nameof(userContext));
    }

    public async Task Handle(UpdateBudgetPeriodCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            budget.UpdateGranularity(request.NewPeriod);
        ...
        catch (HttpRequestException ex)
        {
            ...
            {
                scope.SetTag("handler", "UpdateBudgetPeriodCommandHandler");
                scope.Level = SentryLevel.Error;
            });
            throw new DomainException(
                "A network error occurred while updating the budget. Please try again.");
        }
        catch (Exception ex)
        {
            SentrySdk.CaptureException(ex, scope =>
            ...
            throw new DomainException(
                "An unexpected error occurred while updating the budget. Please try again.");
        }
    }
}
